...
class DLoss(nn.Module):
    ...
    def forward(self, cd_rn, cd_fn):
        ...
        ld_loss = -torch.mean(torch.mean(torch.log(torch.clamp(dra_rn, min=1e-15)), dim=0) +
                              torch.mean(torch.log(torch.clamp(1 - dra_fn, min=1e-15)), dim=0))
        lg_loss = -torch.mean(torch.mean(torch.log(torch.clamp(1 - dra_rn, min=1e-15)), dim=0) +
                              torch.mean(torch.log(torch.clamp(dra_fn, min=1e-15)), dim=0))
        return ld_loss - lg_loss
...
class GLoss(nn.Module):
    ...
    def forward(self, real_image, fake_image, cd_rn, cd_fn):
        ...
        return l1_loss + self.lambda_p * lp_loss + self.lambda_ra * (lg_loss - ld_loss)
